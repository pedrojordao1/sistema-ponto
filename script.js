/ ========================================
    ...
    if (resultado && resultado.dados) {
        ...
        if (chaveCompativel) {
            ...
            // Tentar carregar novamente com a chave exata
            ...
            if (resultadoExato && resultadoExato.dados) {
                ...
                dadosSalvos[chaveData] = dados; // Duplicar para facilitar próximas buscas
                preencherFormulario(dados);
                return;
            }
        }
...
// FUNÇÃO ADICIONAL: Sincronizar dados existentes
// Execute esta função UMA VEZ no console para migrar dados existentes
async function sincronizarDadosExistentes() {
    console.log('Iniciando sincronização de dados...');
    
    // Para cada dia do mês atual, tentar carregar do Google Sheets
    for (let dia = 1; dia <= 31; dia++) {
        ...
        if (resultado && resultado.debug && resultado.debug.chavesEncontradas) {
            const chavesEncontradas = resultado.debug.chavesEncontradas;
            const chaveCompativel = chavesEncontradas.find(chave => {
                const dataExtraida = chave.substring(0, 10);
                return dataExtraida === chaveData;
            ...
            if (chaveCompativel) {
                ...
                if (resultadoExato && resultadoExato.dados) {
                    dadosSalvos[chaveData] = resultadoExato.dados;
                    console.log(`Dados sincronizados para ${dia}/${mesAtual + 1}`);
                }
            }
        }
